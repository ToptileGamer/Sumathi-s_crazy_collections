...
const EMPTY_ADDR = { label: "Home", full_name: "", phone: "", line1: "", city: "", state: "", pincode: "" };
    ...
    <section className="checkout-page">
      ...
      <div className="checkout-layout">
        ...
        <div className="checkout-form">
            ...
            <div className="address-list">
              {addresses.map((addr) => (
                <label key={addr.id}
                  ...
                  <div>
                    <strong>{addr.label}</strong> — {addr.full_name} • {addr.phone}
          ...
          <button className="add-addr-toggle" onClick={() => setShowAddrForm((v) => !v)}>
          ...
          {showAddrForm && (
            <form className="addr-form-checkout" onSubmit={handleSaveAddress}>
              ...
              <input placeholder="Phone Number" required value={addrForm.phone}
                onChange={(e) => setAddrForm((f) => ({ ...f, phone: e.target.value }))} />
              <input placeholder="Address Line" required value={addrForm.line1}
                onChange={(e) => setAddrForm((f) => ({ ...f, line1: e.target.value }))} />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                <input placeholder="City"    required value={addrForm.city}
                  onChange={(e) => setAddrForm((f) => ({ ...f, city: e.target.value }))} />
                <input placeholder="State"   required value={addrForm.state}
                  onChange={(e) => setAddrForm((f) => ({ ...f, state: e.target.value }))} />
              </div>
              <input placeholder="Pincode" required value={addrForm.pincode}
        ...
        <aside className="checkout-summary">
          ...
          <div className="summary-row"><span>GST (3%)</span><span>{formatPrice(tax)}</span></div>
          <div className="summary-divider" />
          <div className="summary-row total"><span>Total</span><span>{formatPrice(total)}</span></div>

          <button className="hero-btn razorpay-btn"
            onClick={handlePlaceOrder} disabled={processing}>
            {processing ? "Opening Payment..." : `Pay ${formatPrice(total)} via Razorpay`}
          </button>
